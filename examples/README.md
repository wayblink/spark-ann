...
| `python/05_pk_preservation.py` | Sets `pk` so search results carry the user's primary key instead of HNSW's internal offset |
...
## Preserving your `pk` column through search results

By default, HNSW assigns sequential internal ids (0, 1, 2, ...) in parquet
row order. The `id` column in search results is that internal id, not any
user-supplied primary key. If you want your own pk back, opt in with
`pk`:
...
spark_ann.build_ann_index(
    ...
    output_path="/tmp/idx",
    config={"pk": "my_pk_col"},   # column must be parquet INT32 or INT64
)
```

The value of that column becomes the HNSW internal id, so search results
carry it back as the `id` column unchanged. See
`examples/python/05_pk_preservation.py` for a runnable demo.

### Limitations

- The `pk` column must be INT32 or INT64. String / UUID columns will raise
  a clear error at build time; mapping-table support for arbitrary pk
...
- Values must be **unique within each local index** (HNSW rejects duplicate
  ...
  a way that could put two rows with the same pk into the same local
  index, deduplicate upstream first.

When `pk` is unset, the previous sequential-offset behavior is preserved
exactly, so existing pipelines keep working.
